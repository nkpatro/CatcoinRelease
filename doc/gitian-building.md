...
*Setup instructions for a Gitian build of Catcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Catcoin
...
- [Building Catcoin](#building-catcoin)
...
Clone the git repositories for catcoin and Gitian and then checkout the catcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the catcoin repository
...
To build Catcoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the catcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/catcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/catcoin-project/catcoin
...
For example:
```bash
URL=https://github.com/user/catcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit catcoin=${COMMIT} --url catcoin=${URL} ../catcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit catcoin=${COMMIT} --url catcoin=${URL} ../catcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit catcoin=${COMMIT} --url catcoin=${URL} ../catcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/catcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/catcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/catcoin-build.assert
```
...
[catcoin/gitian.sigs](https://github.com/catcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
